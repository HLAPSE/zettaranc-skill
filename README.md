...
[![Version](https://img.shields.io/badge/version-4.0.4-green)](docs/CHANGELOG.md)
...
| 3. 本地数据库 | 已执行过同步 | `data/stock_data.db`（SQLite，默认）/ CockroachDB（`CRDB_URL`） | 离线 / 缓存 |
...
bridge（可选）   ──┤   （实例级配置隔离、           ──► 选股 / 分析 / 模拟
                    │    并行安全、pickle 预检）
本地数据库      ──┘   离线兜底（SQLite 默认 / CockroachDB 生产，CRDB_URL 切换）
                    │
...
├── data/                         # 本地数据库与报告（不入库）
│   └── stock_data.db             # SQLite 数据库（15 张核心表 + 4 张自我改进表；生产可切换 CockroachDB，见 CRDB_URL）
├── modules/                      # Python 数据层（约 26,400 行）
│   ├── __init__.py               # 统一 .env 加载入口
│   ├── cli.py / cli_commands.py  # CLI 主入口（15 个顶层子命令）
│   ├── database.py               # 双后端数据库管理（SQLite 默认/回退 + CockroachDB 生产，CRDB_URL 切换，15 张表 + 事务上下文）
...
> 数据库支持双后端：默认 SQLite（`data/stock_data.db`），设置 `CRDB_URL` 后切换到 CockroachDB（生产）。表结构一致，由兼容层自动完成 DDL 转换。
...
| 版本 | 核心变化 |
|------|---------|
| **v4.0.4** | 数据库双后端迁移（SQLite / CockroachDB 兼容层，`CRDB_URL` 切换，占位符/DDL/异常/行访问统一，加 psycopg2-binary 依赖） | ✅ 已完成 |
