...
│   ├── portfolio_diagnosis.py  # 持股检查端到端模块
│   ├── watchlist.py            # 自选股观察池
│   ├── cli.py                  # 命令行工具
...
│   └── test_watchlist.py
└── data/
    └── stock_data.db           # SQLite 数据库（5524 只股票 + 指标缓存）
```
...
# Tushare 中转 API（必填，从环境变量读取，无默认值）
...
│   ├── __init__.py
│   ├── database.py             # SQLite 数据库管理（8 张表）
│   ├── data_sync.py            # 数据同步器（增量/全量，限流 120次/分）
│   ├── indicators/             # 技术指标计算引擎（60+ 指标，4 子模块）
│   │   ├── __init__.py         # 兼容层，保持外部导入不变
│   │   ├── core.py             # 基础类型 + 数学工具 + 核心指标
│   │   ├── price_patterns.py   # 价格形态（双线/单针/砖型图/B1B2B3）
│   │   ├── volume_patterns.py  # 量价信号（卖出评分/交易信号/量比异动）
│   │   └── data_layer.py       # 数据接入 + 缓存层 + 可视化
...
│   ├── trade_parser.py         # 随堂测试解析器
│   ├── trade_manager.py        # 交易记录 CRUD
│   ├── trade_reviewer.py       # 交割单数据准备层
│   └── zettaranc_voice.py      # Z哥语料库 + LLM 提示词模板
...
│   ├── test_trade_manager.py   # 交易记录 CRUD、持仓计算
│   ├── test_portfolio_diagnosis.py # 持股检查端到端
│   └── test_watchlist.py       # 观察池 CRUD、批量扫描
└── references/
    └── research/               # 11 份调研报告
```
...
- [x] **策略回测引擎**：基于指标缓存做历史回测，验证战法胜率（v2.4.0 已完成，支持多策略融合 + 资金曲线）
- [x] **自动选股**：曼城评分体系全市场扫描，每日推荐候选（v2.3.0 已完成，支持 500+ 股票批量扫描）
- [x] **信号推送**：B1/B2/B3 买点触发时自动通知（v2.3.0 已完成，watchlist 扫描支持）
