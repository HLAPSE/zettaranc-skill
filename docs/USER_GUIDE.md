...
核心目标：将 B 站 UP 主 / 前阳光私募冠军基金经理 zettaranc（万千）的投资思维框架、决策启发式和表达 DNA，封装为可供 AI 工具（Claude Code / Cursor / Hermes Agent）调用的 Skill 文件（`SKILL.md`），同时提供基于真实行情数据（BaoStock + AkShare）的 Python 量化分析层。

### 1.1 双模式架构

| 模式 | 环境变量 | 说明 |
|------|---------|------|
| **JNB 模式** | `DATA_MODE=jnb` | 接入真实行情（BaoStock + AkShare），60+ 技术指标实时计算，30+ 战法自动识别，选股、回测、持仓诊断全开 |
...
| 外部数据 | BaoStock（主力，免费无需 Token）+ AkShare（补充，免费无需 Token） |
| 可选数据源 | tushare-data-bridge（HTTP 缓存代理，可选降级） |
| 数据库 | 双后端：SQLite（本地文件，默认/回退）+ CockroachDB（生产，CRDB_URL 启用），15 张表 + 索引 |
| 数据处理 | pandas ≥3.0,<4（标准数据容器，非 Tushare 依赖） |
| 环境配置 | python-dotenv（.env 文件） |
| 测试框架 | pytest（实测 1318 passed, 15 skipped） |
| 版本控制 | Git |
...
```ini
# 数据模式: jnb(走真实行情) 或 websearch(走网络搜索)
DATA_MODE=jnb

# 数据库配置（双后端）
DATA_DIR=data
DB_PATH=data/stock_data.db
# CockroachDB 生产后端（可选，设置后自动切换；需 pip install psycopg2-binary）
# CRDB_URL=postgresql://user:pass@host:26257/zettaranc?sslmode=verify-full

# 可选 HTTP 缓存代理（tushare-data-bridge）
# TUSHARE_BRIDGE_HOST=localhost
# TUSHARE_BRIDGE_PORT=8866
# TUSHARE_BRIDGE_ENABLED=auto
```

> **数据源说明**：本项目默认使用 **BaoStock + AkShare** 双免费数据源，**无需注册或配置任何 Token**，安装即可使用。仅在需要从 bridge 代理拉取缓存数据时配置对应的 `TUSHARE_BRIDGE_*` 项。

### 2.3 验证安装

```bash
# 测试数据源连通性（BaoStock）
python -c "from modules.setup_wizard import test_baostock_connection; print(test_baostock_connection())"
...
| `CRDB_URL` | 否 | 无 | CockroachDB 连接串，设置后数据库切到 CockroachDB 生产后端（需 `psycopg2-binary`） |
| `TUSHARE_BRIDGE_HOST` / `TUSHARE_BRIDGE_PORT` / `TUSHARE_BRIDGE_TIMEOUT` | 否 | `localhost` / `8866` / `30` | tushare-data-bridge HTTP 缓存代理配置（可选降级源） |
| `TUSHARE_BRIDGE_ENABLED` | 否 | `auto` | bridge 降级开关：`auto` / `always` / `never` |
| `DB_PATH` | 否 | `data/stock_data.db` | SQLite 数据库路径，支持绝对/相对路径（CockroachDB 模式下忽略） |
...
```bash
# 切换到 websearch 模式（纯对话）
python -c "from modules.setup_wizard import write_env_file; write_env_file(mode='websearch')"

# 切换回 JNB 模式（真实行情，默认 BaoStock + AkShare 双免费数据源）
python -c "from modules.setup_wizard import write_env_file; write_env_file(mode='jnb')"
```
...
- `sync_log`：数据同步日志
- `watchlist`：自选股观察池
- `tushare_indicator_cache`：外部数据源官方指标缓存（与自研指标做 diff 验证）
...
> ⚠️ 全量同步需要较长时间（约 50 分钟），因为要对每只股票调用 API。默认数据源优先级见 3.2 节（BaoStock → AkShare → bridge → 本地数据库）。
...
```bash
# 全部测试（当前实测：1318 passed, 15 skipped，约 30 秒）
...
# 真实数据回归（需配置真实数据源，默认走 BaoStock/AkShare）
python -m pytest tests/test_indicators_realdata.py -v
```

### 17.2 测试覆盖范围

当前共 98 个测试文件，覆盖：数据库与数据同步、60+ 指标计算、30+ 战法识别、选股评分、回测框架（单股/多策略融合/组合/网格寻优/ATR 止损）、模拟器（12 个文件）、verify v1.0 验收（10 个文件）、Darwin 自优化管线、CLI 子命令分发、监控推送、跟踪系统等。代表性文件：
...
A: 检查以下几点：
1. `.env` 中 `DATA_MODE` 是否设为 `jnb`
2. 默认数据源 BaoStock / AkShare 是否正常（免费无需 Token，无需额外配置）
3. 若配置了 `TUSHARE_BRIDGE_*`，检查 bridge 代理是否可访问
4. 网络是否正常（如 `ping baostock.com`）
...
A: 全量 5500 只股票约需 50 分钟。这是正常现象，数据源有内置限流与重试。

**Q: 偶发超时怎么办**

A: 数据源偶尔返回 Read timeout 或维护中，增量同步会自动跳过，下次同步时补上。
...
> **双后端说明**：默认 SQLite（`data/stock_data.db`），设置 `CRDB_URL` 后切到 CockroachDB（生产）。表结构一致，由 `modules/database.py` 兼容层自动完成 DDL 转换，调用方代码无需区分。
...
#### tushare_indicator_cache（外部指标缓存）

外部数据源官方指标缓存（macd_dif、rsi_6、kdj_k、boll_mid 等），用于与自研指标做 diff 验证。
...
| MINOR | 新增战术/启发式/语料/模块 | v3.8.0：新增 BaoStock/AkShare 双免费数据源接入 |
...
- 数据库路径统一从 `DB_PATH` / `CRDB_URL` 环境变量读取（由兼容层屏蔽后端差异）
- 错误处理返回空 DataFrame/None 而非抛异常

## 附录 D：安全与合规

1. 此项目**不构成任何投资建议**，金融市场风险极高
2. 数据源凭证（如有）通过 `.env` 管理，绝不硬编码
